...
void CurlInvoker::ExecuteRequest(CurlHandler& curl_handler) const {
  ...
  if (CURLE_OK != curl_ret) {
    const char* errr = curl_easy_strerror(curl_ret);
    api_.Logger_LogMessage(&Logger(), ORT_LOGGING_LEVEL_ERROR, ("Curl error code: " + std::to_string(curl_ret)).c_str(), __FILE__, __LINE__, __FUNCTION__);
